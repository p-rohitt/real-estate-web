...
$query=mysqli_query($con, "SELECT * FROM `property` WHERE agent_id='$uid' and status = 'Available'");
while($row=mysqli_fetch_array($query)) {
    ?>
                            <tr>
                                <td>
                                    <?php echo '<img src="data:image/jpeg;base64,' . base64_encode($row[14]) . '"/>'; ?>
                                        </td>
                                <td>
									
                                    <div class="property-info d-table">
                                        <h5 class="text-secondary text-capitalize"><a href="propertydetail.php?pid=<?php echo $row['0'];?>"><?php echo $row['3'];?></a></h5>
                                        <span class="font-14 text-capitalize"><i class="fas fa-map-marker-alt text-dark font-13"></i>&nbsp; <?php echo $row['5'];?></span>
                                        <div class="price mt-3">
											<span class="text-dark">₹&nbsp;<?php echo $row['4'];?></span>
										</div>
                                    </div>
								</td>
                                <td><?php echo $row['8'] . " bhk";?></td>
                                <td class="text-capitalize"> <?php echo $row['6'] .", ". $row['7'];?></td>
                                <td><?php echo "For ".$row[11];?></td>
								<td class="text-capitalize"><?php
                                $sqlquery = "SELECT ROUND(DATEDIFF(NOW(),'$row[12]')/365,1)";
    $sq = mysqli_query($con, $sqlquery);
    $row2 = mysqli_fetch_array($sq);

    echo  $row2[0] . " years" ?> </td>
