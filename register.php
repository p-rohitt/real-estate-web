...
if($utype == 'agent') {
	$query = "SELECT * FROM agent where contact='$email'";
    ...
    $sql = "INSERT INTO agent(password,agent_name,contact) VALUES ('$pass','$name','$email')";
	...
	if($num == 1)
	{
		$error = "<p class='alert alert-warning'>Email Id already Exist</p> ";
				...
				$sql = "INSERT into user(`user_name`,`user_email`,`user_pass`,`user_phone`) values ('$name','$email','$pass','$phone')";
				$query = mysqli_query($con,$sql);
                if($query) {
					$msg = "<p class='alert alert-success'>Registered successfully</p> ";
				} else {
			...
			elseif($utype == 'seller'){
				$query = "SELECT * FROM seller where contact='$email'";
	$res=mysqli_query($con, $query);
	$num=mysqli_num_rows($res);
	...
	if($num == 1)
	{
		$error = "<p class='alert alert-warning'>Email Id already Exist</p> ";
				...
				$sql = "INSERT into seller(`seller_name`,`contact`,`password`) values ('$name','$email','$pass')";
				$query = mysqli_query($con,$sql);
                if($query) {
					$msg = "<p class='alert alert-success'>Registered successfully</p> ";
				} else {
				...
				$msg = "<p class = 'alert alert-warning'> Choose one role </p>";
			}
                        ...
                        <div class="login-right">
							<div class="login-right-wrap">
								...
								<form method="post" enctype="multipart/form-data">
									...
									<button class="btn text-white" style = "background-color:black;margin-left:100px;margin-top:30px;" name="reg" value="Register" type="submit">Register</button>
